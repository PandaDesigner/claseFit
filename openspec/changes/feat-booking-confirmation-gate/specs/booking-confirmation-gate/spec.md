# booking-confirmation-gate Specification (delta)

## Purpose

Introduces a presentation-level confirmation gate that the user must explicitly approve before a booking is persisted. Mirrors the `CancellationSheet` pattern from `feat-modal-polish` (compound `Root` + `Title` + `Description` + `Actions`, optional `SessionPreview`-style card, platform `<Modal>`, exported `*Content` for tests).

The gate is presentation-only — it does not own any business decision. Validation of capacity, duplicates, and daily limits continues to live in the `BookClass` use case (see `class-booking` spec, Requirement: Book a class session).
