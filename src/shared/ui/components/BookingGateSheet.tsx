...
import { type SessionPreview } from './CancellationSheet';

// Re-export for callers that only want the type (mirrors CancellationSheet's public surface).
export type { SessionPreview };

export interface BookingGateSheetProps {
...
interface BookingGateSheetContextValue {
  readonly onCancel: () => void;
  readonly onConfirm: () => void;
}

const Context = createContext<BookingGateSheetContextValue | null>(null);

function useSheet(): BookingGateSheetContextValue {
  const value = useContext(Context);
  if (!value) {
    throw new Error('BookingGateSheet parts must be rendered inside BookingGateSheet.Root');
  }
  return value;
}
...
function Root({
  ...
  children,
  testID,
}: BookingGateSheetProps) {
  const value = useMemo<BookingGateSheetContextValue>(
    () => ({ onCancel, onConfirm }),
    [onCancel, onConfirm],
  );
  return (
...
export interface BookingGateSheetContentProps {
  readonly onCancel: () => void;
  readonly onConfirm: () => void;
  readonly sessionPreview?: SessionPreview | null;
  readonly children: ReactNode;
 ...
 */
export function BookingGateSheetContent({
  onCancel,
  onConfirm,
  sessionPreview,
  children,
  testID,
}: BookingGateSheetContentProps) {
  const value = useMemo<BookingGateSheetContextValue>(
    () => ({ onCancel, onConfirm }),
    [onCancel, onConfirm],
  );
...
function Title() {
  return <Text style={styles.title}>{'¿Reservar esta clase?'}</Text>;
}

function Description() {
  return (
    <Text style={styles.description}>
      {'Tu cupo quedará guardado hasta 2 horas antes de empezar.'}
    </Text>
  );
}
  ...
  return (
    <View style={styles.actions}>
      <Pill
        ...
        accessibilityLabel="Volver a la lista de clases"
